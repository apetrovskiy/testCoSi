...
public class GroupCalculator {
  int[][] calculate(int[][] a) {
    ...
    result
        ...
        .forEach(
            item ->
                System.out.println("first = " + item.getKey() + ", value = " + item.getValue()));
  ...
  int[] calculate1(int[][] a) {
    ...
    final Map<Integer, Integer> result =
        IntStream.range(0, a.length)
            ...
            // .collect(Collectors.groupingBy(Function.identity(), Collectors.toMap(index -> index,
            // index -> index)));
            // .collect(Collectors.groupingBy(Function.identity(), Collectors.toMap(index -> index,
            // index -> a[index])));
            // double
            // .collect(Collectors.groupingBy(Function.identity(), Collectors.toMap(index -> index,
            // index -> a[index][0])));
            // .collect(Collectors.groupingBy(Function.identity(), Collectors.summingInt(index ->
            // a[index][0])));
            .collect(
                Collectors.groupingBy(
                    Function.identity(),
                    Collectors.summingInt(
                        index -> Arrays.stream(a[index]).sum() / a[index].length)));
    System.out.println("============================================================");
